"""
TradeStation API 自定义封装库,v2版本实现,官方现在也开发出v3版本,当前v2,v3版本共存
"""
...
class TradeStationClient():
    ...
    def login(self) -> bool:
        ...
        if self.config['cache_state'] and self.state['refresh_token']:
    ...
    def _token_validation(self, nseconds: int = 5) -> None:
        ...
        if self._token_seconds() < nseconds: #token剩余时间不足5秒
            if self.config['refresh_enabled']: #如果允许刷新token就重新刷新
                return self._grab_refresh_token()
            else: #否则就直接返回token无效
                return False
        else:
            return True #token剩余时间充足,token有效
    ...
    def grab_access_token(self, code) -> bool:
        """
        通过授权code去获取真正的访问令牌(token),并保存到文件,用于web授权服务
        """
    ...
    def silent_sso(self) -> bool:
        """
        检测访问令牌(token)是否过期,如果要过期的话就重新刷新token,用于token定时监测程序
        """
        ...
        if self.config['cache_state'] and self.state['refresh_token']:
            ...
            # if it was successful, the user is authenticated.
            if self._silent_sso(): #函数内部会检测访问令牌(token)是否过期,如果要过期的话就重新刷新token
